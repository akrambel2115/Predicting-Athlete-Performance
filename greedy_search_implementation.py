...
class Node:
    ...
    Attributes:
        ...
        cost: The cost to reach this state
...
class AthletePerformanceProblem:
    ...
    def expand_node(self, node, use_cost=False, use_heuristic=False):
        ...
        for action in self.actions(node.state):
            ...
            if self.is_valid(new_state):
                # Calculate cost and heuristic values if requested
    ...
    def cost(self, state):
        ...
        This function evaluates how "good" a state is in terms of the goal.
        Lower cost means the state is better for achieving the goal.
        The cost is based on performance deficit, risk level, and fatigue level.
        
        Args:
            state: The state to evaluate
            
        Returns:
            A numerical cost value (lower is better)
        """
        ...
        # Normalized risk factor (higher risk = higher cost)
        risk_factor = risk / self.max_risk if self.max_risk > 0 else 0
        
        # Normalized fatigue factor (higher fatigue = higher cost)
        fatigue_factor = fatigue / self.max_fatigue if self.max_fatigue > 0 else 0
        
        # Weighted cost function - balance performance improvement with risk/fatigue management
        # Lower cost for higher performance and lower risk/fatigue
        cost = (
            5.0 * perf_deficit +        # Performance deficit has highest weight
            2.0 * risk_factor +         # Risk is important to avoid injuries
            1.5 * fatigue_factor        # Fatigue management is important but secondary to risk
        )
